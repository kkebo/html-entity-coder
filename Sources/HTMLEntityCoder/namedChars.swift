...
extension Character {
    fileprivate init?(from scalars: (Unicode.Scalar, Unicode.Scalar)) {
        switch scalars {
        case (let c1, "\0"): self.init(c1)
        case ("\u{66}", "\u{6A}"): return nil
        case ("\u{205F}", "\u{200A}"): return nil
        case (let c1, let c2): self.init(String(String.UnicodeScalarView([c1, c2])))
        }
    }
}
...
let namedChars: [String: (Unicode.Scalar, Unicode.Scalar)] = {
    var dict: [String: (Unicode.Scalar, Unicode.Scalar)] = .init(
        minimumCapacity: namedCharsPart1.count + namedCharsPart2.count + namedCharsPart3.count
    )
    for i in namedCharsPart1.indices {
        let (key, v0, v1) = namedCharsPart1[i]
        dict[key] = (v0, v1)
    }
    for i in namedCharsPart2.indices {
        let (key, v0, v1) = namedCharsPart2[i]
        dict[key] = (v0, v1)
    }
    for i in namedCharsPart3.indices {
        let (key, v0, v1) = namedCharsPart3[i]
        dict[key] = (v0, v1)
    }
    return dict
}()

let namedCharsPart1: [1000 of (String, Unicode.Scalar, Unicode.Scalar)] = [
    ("Aacute;", "\u{C1}", "\0"),
    ("aacute;", "\u{E1}", "\0"),
    ("Abreve;", "\u{102}", "\0"),
    ("abreve;", "\u{103}", "\0"),
...
let namedCharsPart2: [1000 of (String, Unicode.Scalar, Unicode.Scalar)] = [
    ("lsquo;", "\u{2018}", "\0"),
    ("lsquor;", "\u{201A}", "\0"),
    ("Lstrok;", "\u{141}", "\0"),
    ("lstrok;", "\u{142}", "\0"),
...
let namedCharsPart3: [231 of (String, Unicode.Scalar, Unicode.Scalar)] = [
    ("vcy;", "\u{432}", "\0"),
    ("VDash;", "\u{22AB}", "\0"),
    ("Vdash;", "\u{22A9}", "\0"),
    ("vDash;", "\u{22A8}", "\0"),
